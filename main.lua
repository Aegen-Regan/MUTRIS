...
-- ==== Tracking de velocidad de caída combinada ====
local lock_events = {}

local function trackLock()
    table.insert(lock_events, love.timer.getTime())
end

function _G.NotifyPieceLock()
    trackLock()
end
...
local function getCombinedDropRate()
    ...
    while i <= #lock_events do
        if now - lock_events[i] > window then
            table.remove(lock_events, i)
        else
            i = i + 1
        end
    end
...
function love.load()
    ...
    MusicManager.loadLayers({
        "music/layer1_calm.ogg",
        "music/layer2_tension.ogg",
        "music/layer3_climax.ogg",
    })
...
function love.update(dt)
    ...
    if game_state == "play" and player and player.grid then
        for r = 21, 32 do
            for c = 1, 10 do
                if player.grid[r] and player.grid[r][c] ~= 0 then
                    danger = math.max(danger, (33 - r) / 12)
                end
            end
        end
    end
...
function love.draw()
    ...
    if game_state == "play" and _G.AudioBeatPulse and _G.AudioBeatPulse > 0 then
        ...
        if active_punch >= 0.50 then
            local tier_factor = (active_punch - 0.50) / 0.50
            vertical_bounce = _G.AudioBeatPulse * (AudioManager.zone_active and 8 or 4) * tier_factor
        end
        love.graphics.translate(0, vertical_bounce)
    end
...
function love.keypressed(key)
    if game_state == "menu" then
        ...
        elseif key == "down" or key == "kp2" then
            selected_diff = math.min(#difficulties, selected_diff + 1)
