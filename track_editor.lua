...
-- Control del Laboratorio y Cola Batch
TrackEditor.active = false
TrackEditor.batch_queue = {} -- Lista de archivos encolados por arrastre masivo
...
-- Simulación de Espectrograma RMS por Hardware-Sync (Windows Media Player Vibe)
TrackEditor.spectrogram_bars = {}
for i = 1, 32 do TrackEditor.spectrogram_bars[i] = 0 end

-- Estados de Menús Desplegables (Dropdowns con Mouse)
TrackEditor.dropdowns = {
    note = { active = false, x = 320, y = 245, w = 120, h = 22 },
    mode = { active = false, x = 320, y = 295, w = 120, h = 22 }
}
TrackEditor.selected_field = 1 -- Para soporte híbrido de teclado
...
-- Iniciar procesamiento, soporta inyección individual o masiva (Batch)
...
function TrackEditor.update(dt)
    ...
    -- CONTROL DE RÁFAGA CONTINUA (DAS/ARR) PARA BPM POR TECLADO
    ...
    -- MOTOR DE ESPECTROGRAMA AUDIO-DRIVEN (WINDOWS MEDIA VISUALIZER VIBE):
    -- Si la música suena, calculamos la energía matemática oscilante basada en la posición exacta 
    -- de hardware del stream para simular picos RMS y alimentar el parpadeo.
    if TrackEditor.is_playing and TrackEditor.preview_source then
        ...
        -- Detección rítmica limpia para el flash perimetral
        if fraction < 0.08 and TrackEditor.flash_alpha <= 0.1 then
            TrackEditor.flash_alpha = 0.45
            AudioManager.playHatClosed(0.08)
        end
        
        -- Generar las 32 barras dinámicas del ecualizador rítmico en base al pulso de la canción
        for i = 1, 32 do
            ...
            -- Aplicar interpolación lineal suave para evitar tirones en las barras
            TrackEditor.spectrogram_bars[i] = TrackEditor.spectrogram_bars[i] + (target_height - TrackEditor.spectrogram_bars[i]) * 15 * dt
        end
    else
        -- Decaimiento natural de las barras si la música está en pausa
        for i = 1, 32 do
...
function TrackEditor.mousepressed(mx, my, button)
    ...
    -- 1. CONTROL DE DESPLEGABLE DE NOTAS (CHROMATIC KEY)
    ...
    -- 2. CONTROL DE DESPLEGABLE DE MODO (MAJOR / MINOR)
    ...
    -- 3. BOTÓN INTERACTIVO: PLAY / PAUSA
    ...
    -- 4. BOTÓN INTERACTIVO: CONFIRMAR E INYECTAR
...
function TrackEditor.draw()
    ...
    -- RENDERIZADO DEL ESPECTROGRAMA ESTILO WINDOWS MEDIA PLAYER
    -- Dibuja las 32 barras de frecuencia en la base inferior del editor en caliente
    ...
    love.graphics.printf("SOUNDTRACK INJECTION LAB", 0, 25, 800, "center")
    love.graphics.setFont(love.graphics.newFont(12))
    love.graphics.setColor(0.5, 0.5, 0.6)
    love.graphics.printf("INTERACTIVE BATCH MODE SYSTEM  |  MOUSE DROPDOWNS INTEGRATED", 0, 58, 800, "center")

    -- Panel del Archivo Actual
    love.graphics.setColor(0.03, 0.04, 0.08, 0.8)
    love.graphics.rectangle("fill", 150, 85, 500, 50, 4)
    love.graphics.setColor(0, 0.8, 1, 0.2)
    love.graphics.rectangle("line", 150, 85, 500, 50, 4)
    
    love.graphics.setFont(love.graphics.newFont(10))
    love.graphics.setColor(0.4, 0.6, 0.8)
    love.graphics.print("TARGET BATCH TRACK:", 170, 93)
    love.graphics.setFont(love.graphics.newFont(13))
    love.graphics.setColor(1, 1, 1)
    love.graphics.print(TrackEditor.track_name, 170, 110)

    -- RENDERIZADO DE CAMPOS DE EDICIÓN ESTÁTICOS
    love.graphics.setFont(love.graphics.newFont(12))
    love.graphics.setColor(0.6, 0.7, 0.8)
    
    -- Fila 1: BPM Slider manual / teclado
    love.graphics.print("BPM (BEATS PER MINUTE):", 170, 165)
    love.graphics.setColor(0.05, 0.07, 0.12, 0.9)
    love.graphics.rectangle("fill", 320, 160, 200, 24, 2)
    love.graphics.setColor(1, 1, 1)
    love.graphics.printf(tostring(TrackEditor.bpm) .. " BPM", 320, 165, 200, "center")
    love.graphics.setFont(love.graphics.newFont(10))
    love.graphics.setColor(0, 0.8, 1, 0.4)
    love.graphics.print("[Hold Left/Right to slide or Tap Spacebar]", 535, 166)

    -- Fila 2: Cuadro Dropdown Cromático de Notas
    love.graphics.setFont(love.graphics.newFont(12))
    love.graphics.setColor(0.6, 0.7, 0.8)
    ...
    -- Fila 3: Cuadro Dropdown de Modo Camelot (Mayor / Menor)
    love.graphics.setColor(0.6, 0.7, 0.8)
    ...
    -- RENDERIZADO DE BOTONES INTERACTIVOS DE BASE (PLAY / CONFIRM)
    ...
    -- CAPAS FLOTANTES DE MENÚS DESPLEGABLES (Dropdown overlays sobre la UI)
    -- Dropdown de Notas desplegado en caliente
    ...
    -- Dropdown de Modos desplegado en caliente
    ...
    -- Footer informativo de lotes
    love.graphics.setFont(love.graphics.newFont(11))
    love.graphics.setColor(0, 0.8, 1, 0.35)
    love.graphics.printf("REMAINING BATCH TRACKS IN COLA: " .. #TrackEditor.batch_queue, 0, 565, 800, "center")
...
function TrackEditor.saveAndInject()
    if TrackEditor.current_file_path then
        -- Pasamos las variables fijas al TrackManager que inyectará el bloc de notas
        local success, msg = TrackManager.injectCustomTrack(
            ...
            TrackEditor.mode,
            0, -- Forzamos cero como bandera ya que el espectrograma RMS se calcula solo
            0
        )
        if success then
            -- Pasar de inmediato a procesar el siguiente tema de la cola de lotes
            TrackEditor.processNextInQueue()
        end
    end
end
