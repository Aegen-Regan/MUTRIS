...
-- ============================================================================
-- MUTRIS ENGINE: BEAT-LOCK TIMING ENGINE (FASE 7)
-- Arquitectura: Zero-GC / Hardware Clock Anclado / Groove Strikes
-- ============================================================================
local BeatLock = {}
...
-- Estructura de estado estática pre-alocada por jugador
...
-- ⚡ EVALUACIÓN DETERMINISTA DEL HARD DROP
function BeatLock.evaluate(board, is_resonance_stance)
    ...
    -- Distancia en segundos al pulso más cercano (hacia adelante o atrás)
    local dist_to_nearest_beat = math.min(fraction, 1.0 - fraction) * beat_duration
    board.last_lock_timing_delta = dist_to_nearest_beat

    -- Obtener ventana calibrada por ARCHON (o bonus de Stance Resonance)
    ...
    if dist_to_nearest_beat <= window then
        ...
        -- Rampa de energía y transitorio de graves
        _G.AudioBeatPulse = 1.0
        AudioManager.playSubBassThud(2)
        return true, MetaBalancer.get("beat_bonus_attack") or 1
    else
        -- Desfase: Reinicia la racha de Groove
        board.beat_lock_hit = false
        board.groove_streak = 0
        board.groove_multiplier = 1.0
        return false, 0
