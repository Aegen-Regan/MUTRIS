...
function Piece.new(id, board)
    ...
    self.shape = SRS.shapes[id] or { {{1,1,1,1}} }
...
function Piece:lock()
    ...
    for r = 1, #shape do
        for c = 1, #shape[r] do
            if shape[r][c] ~= 0 then
                local ty = self.y + r - 1
                if ty >= 1 and ty <= 40 then self.board.grid[ty][self.x + c - 1] = self.id end
            end
        end
    end
    self.locked = true
...
function Piece:draw(bx, by)
    ...
    -- --- GHOST PIECE: DEFORMACIÓN Y COLOR INTENSO ---
    if self.board and bx == self.board.x and by == self.board.y then
        ...
        -- La sombra es MÁS colorida y brillante que la pieza real
        local ghost_a = 0.15 + (energy * 0.2) + (pulse * 0.2)
        
        -- Efecto de "deformación" (vibración por beat)
        local deform = pulse * (2 + energy * 4)
        
        for r = 1, #shape do
            for c = 1, #shape[r] do
                if shape[r][c] ~= 0 then
                    local px = bx + (self.x + c - 2) * 24
                    local py = by + (gy + r - 22) * 24
                    
                    -- Dibujar relleno brillante pulsante
                    love.graphics.setColor(clr[1], clr[2], clr[3], ghost_a * 0.4)
                    love.graphics.rectangle("fill", px + 1 - deform/2, py + 1 - deform/2, 22 + deform, 22 + deform)
                    
                    -- Dibujar borde "eléctrico" que se deforma
                    love.graphics.setLineWidth(1 + pulse * 2)
                    love.graphics.setColor(clr[1], clr[2], clr[3], ghost_a)
                    love.graphics.rectangle("line", px + 1 - deform, py + 1 - deform, 22 + deform*2, 22 + deform*2)
                end
            end
        end
    end

    -- --- PIEZA REAL (ESTILO DISCO) ---
    for r = 1, #shape do
        for c = 1, #shape[r] do
            if shape[r][c] ~= 0 then
