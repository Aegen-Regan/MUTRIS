...
function PartBreaking.registerLineClear(player_board, cleared_count, is_tspin)
    ...
    if PartBreaking.parts.core.broken then base_damage = base_damage * 1.5 end

    local part_dmg = math.floor(base_damage * 0.40)
...
function PartBreaking.drawIndicators(boss_board)
    ...
    local bs = boss_board.block_size or 24
    ...
    love.graphics.line(bx, by - 4, bx + 3 * bs, by - 4)
    love.graphics.line(bx, by - 4, bx, by)
    love.graphics.line(bx + 3 * bs, by - 4, bx + 3 * bs, by)
    ...
    love.graphics.line(bx + 3 * bs + 2, by - 4, bx + 7 * bs, by - 4)
    love.graphics.line(bx + 3 * bs + 2, by - 4, bx + 3 * bs + 2, by)
    love.graphics.line(bx + 7 * bs, by - 4, bx + 7 * bs, by)
    love.graphics.print(c_broken and "[EXPOSED]" or string.format("CORE %d%%", math.floor(c_pct * 100)), bx + 3 * bs + 10, by - 14)
    ...
    love.graphics.line(bx + 7 * bs + 2, by - 4, bx + 10 * bs, by - 4)
    love.graphics.line(bx + 7 * bs + 2, by - 4, bx + 7 * bs + 2, by)
    love.graphics.line(bx + 10 * bs, by - 4, bx + 10 * bs, by)
    love.graphics.print(t_broken and "[SEVERED]" or string.format("TAIL %d%%", math.floor(t_pct * 100)), bx + 7 * bs + 10, by - 14)
